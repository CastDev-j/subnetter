    ...
    <section className="mb-12">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {subnets.map((subnet) => (
              <tr key={subnet.index} className="border-b border-primary-900">
                <td className="py-3 pr-4 text-primary-500">{subnet.index}</td>
                <td className="py-3 pr-4 text-primary-100">{subnet.networkAddress}/{subnet.cidr}</td>
                <td className="py-3 pr-4 text-primary-400">{subnet.firstHost}</td>
                <td className="py-3 pr-4 text-primary-400">{subnet.lastHost}</td>
                <td className="py-3 pr-4 text-primary-400">{subnet.broadcastAddress}</td>
                <td className="py-3 pr-4 text-primary-500">{subnet.mask}</td>
                <td className="py-3 text-right text-primary-300">{subnet.usableHosts}</td>
              </tr>
            ))}
          </tbody>
        </table>
